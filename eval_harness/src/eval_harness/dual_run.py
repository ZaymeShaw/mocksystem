...
Entry:
  PYTHONPATH=src python3 -m eval_harness.dual_run --bundle … --cases A01,A02
...
MOCK_SYSTEM_ROOT = Path("/Users/xiaozijian/WorkSpace/package/mock_system")
...
DEFAULT_CLAUDE_CONFIG = EVAL_HARNESS_ROOT / "config.yaml"
DEFAULT_PI_CONFIG = EVAL_HARNESS_ROOT / "config_pi.yaml"
...
def _run_insurance(*, dual_root: Path, bundle: Path, case_ids: list[str], resume: bool = False) -> dict[str, Any]:
    ...
    try:
        import httpx  # noqa: F401
        from eval_harness.insurance_qa_adapter import run_live_batch
    ...
    cmd = [
        py,
        "-m",
        "eval_harness.insurance_qa_adapter",
        "--live-batch",
        "--cases",
        ",".join(case_ids),
        "--run-id",
    ...
    try:
        ...
        if proc_rc != 0:
            err = f"insurance_qa_adapter exit={proc_rc}"
    except Exception as e:
        err = repr(e)
        traceback.print_exc()
...
def build_arg_parser() -> argparse.ArgumentParser:
    ...
    p.add_argument(
        ...
        help="Pi runner config (profiles/pi_coding_local_relay.yaml). Default config_pi.yaml",
    )
    p.add_argument(
        "--agents",
        type=str,
...
def main(argv: Optional[list[str]] = None) -> int:
    ...
    manifest: dict[str, Any] = {
        ...
        "notes": (
            "Production dual/triple entry (eval_harness.dual_run). One stamp; selected "
    ...
    try:
        ...
        settings = MOCK_SYSTEM_ROOT / "mock_run" / ".claude" / "settings.local.json"
