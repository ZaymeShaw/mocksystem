...
`config.yaml` / 默认 dual 仍指向 v1，**故意不改**（保护在飞 Claude 跑）。
...
PYTHONPATH=src python3 -m eval_harness.dual_run \
  ...
  --claude-config config_claude_0922.yaml \
  --pi-config config_pi_0922_shared.yaml \
...
- Pi：`cwd=pi_run`，`ITOOLS_AGENT_ID=pi`，local-relay，`thinking=off`（profile 现状，未改 models.json）
...
# Pi only smoke
PYTHONPATH=src python3 -m eval_harness.dual_run \
  --agents pi --pi-config config_pi_0922_shared.yaml \
  --bundle bundles/120_prompt_only_0922_shared.jsonl \
  --cases A01 --stamp my_pi_smoke

# Claude+Insurance on 0922（不碰默认 config.yaml）
PYTHONPATH=src python3 -m eval_harness.dual_run \
  --bundle bundles/120_prompt_only_0922_shared.jsonl \
  --claude-config config_claude_0922.yaml \
